...
def extract_plotly_chart(chart_html: str) -> str:
    """Extract just the chart div and script from Plotly's full HTML output.
    
    Plotly's to_html() returns a complete HTML document, but we need just
    the div and script content to embed properly in our page without nested HTML.
    """
    import re
    ...
    # Combine div and script without the <script> tags
...
def wrap_chart_in_div(chart_html: str, title: str = "") -> str:
    ...
    Note: expects already-extracted chart content (div + script), not full HTML.
    """
    html_parts = []
...
# ======================
# Note: Chart Generation Functions have been moved to dashboard_utils.py
# and 4_weekly_report.py implements its own weekly-specific chart functions.
# ======================
