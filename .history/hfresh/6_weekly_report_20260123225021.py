...
def generate_markdown_report(conn: sqlite3.Connection) -> str:
    ...
    difficulty = get_difficulty_breakdown(conn, pull_date)
    ...
    # Difficulty
    lines.append(f"## 📊 Recipe Difficulty")
    lines.append(f"")
    if difficulty and difficulty.get('avg') is not None:
        lines.append(f"- **Average Difficulty:** {difficulty.get('avg')}/5")
        lines.append(f"- **Range:** {difficulty.get('min')} - {difficulty.get('max')}")
    else:
        lines.append(f"*No difficulty data available*")
    lines.append(f"")
