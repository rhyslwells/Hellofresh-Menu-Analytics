...
Purpose
-------
Fetches weekly menus (with embedded recipes) from the hfresh API.
Designed to run weekly to track menu composition over time.

Architecture
------------
- Bronze layer: Raw JSON snapshots (append-only)
- Fetches menus with recipes embedded (efficient, single API call)
- Reference data: Fetched on first run only

Data Model
----------
- Menu = Weekly meal plan (contains multiple recipes)
- Recipe = Individual dish (embedded in menu response)
...
def fetch_menus_for_week(
    ...
    """
    print(f"  Fetching menus for week: {start_date.date()} to {end_date.date()}")
    ...
    total_recipes = sum(len(menu.get("recipes", [])) for menu in menus)
    print(f"  → {len(menus)} menus containing {total_recipes} total recipes")
